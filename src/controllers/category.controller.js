...
const modifyCategory = asyncHandler(async (req, res) => {
  ...
  if (!oldTitle || oldTitle === "") throw new ApiError(400, "Old Category name required");
  if (!title || title === "") throw new ApiError(400, "Category name required");
  ...
  if (!category) throw new ApiError(500, "Error occured while updating the category name!");
